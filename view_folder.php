...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file'])) {
    ...
    if (move_uploaded_file($_FILES['file']['tmp_name'], $target_file)) {
        ...
        if (file_exists($jsonFile) && ($subfolder === 'absensi' || $subfolder === 'dokumentasi')) {
            ...
            // Add new file path (relative: absensi/filename.jpg)
            $relPath = $subfolder . '/' . basename($_FILES['file']['name']);
            if (!in_array($relPath, $jsonData[$key])) {
                $jsonData[$key][] = $relPath;
    ...
    if (file_exists($file_path)) {
        ...
        if (file_exists($jsonFile) && ($sub === 'absensi' || $sub === 'dokumentasi')) {
            ...
            if (isset($jsonData[$key])) {
                $relPath = $sub . '/' . $file;
                // Remove from array (re-index to avoid gaps)
                $jsonData[$key] = array_values(array_filter($jsonData[$key], fn($p) => $p !== $relPath));
                file_put_contents($jsonFile, json_encode($jsonData, JSON_PRETTY_PRINT));
            }
        }
        // ------------------------------
...
// Check for JSON Data
$json_undangan = file_exists($folder_path . 'undangan.json');
$json_notulensi = file_exists($folder_path . 'notulensi.json');
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .btn-action-upload {
            background: linear-gradient(135deg, #a0b2eeff, #2563eb);
            /* Modern Blue */
        }
...
<body>
    ...
    <div class="container">
        <!-- Sidebar & Main Content Header (unchanged) -->
        <div class="sidebar">
            <h2>UANG</h2>
            <ul>
        ...
        <div class="main-content">
            ...
            <div class="files-grid">
                ...
                <div class="file-section">
                    <div class="fs-header">
                        ...
                        <div style="margin-left:auto; display:flex; gap:8px; align-items:center;">
                            <button onclick="triggerUpload('undangan')" class="btn-action btn-action-upload" title="Upload File">
                ...
                <div class="file-section">
                    <div class="fs-header">
                        ...
                        <div style="margin-left:auto; display:flex; gap:8px; align-items:center;">
                            <button onclick="triggerUpload('notulensi')" class="btn-action btn-action-upload" title="Upload File">
                                <i class="fas fa-cloud-upload-alt"></i> Upload
                            </button>
            ...
            <form id="uploadForm" method="post" enctype="multipart/form-data" style="display:none;">
                <input type="hidden" name="subfolder" id="uploadSubfolder">
                <input type="file" name="file" id="uploadFile" onchange="document.getElementById('uploadForm').submit()">
            </form>

            <script>
                function triggerUpload(subfolder) {
                    document.getElementById('uploadSubfolder').value = subfolder;
